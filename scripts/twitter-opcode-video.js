...
const dir='artifacts/raw/opcode-video';await mkdir(dir,{recursive:true});
const browser=await chromium.launch();
const shots=[
 {id:'choose',initial:0,input:1,title:'ADD computes.',accent:'CHOOSE understands.',delay:1000},
 {id:'score',initial:0,input:1,title:'SCORE measures meaning.',accent:'Code gets a number.',delay:1300},
 {id:'test',initial:0,input:1,title:'TEST asks yes or no.',accent:'Code chooses the branch.',delay:1050},
 {id:'batch',initial:0,input:2,title:'JUDGE asks in parallel.',accent:'One typed answer bank.',delay:850},
];
const css=`
...
`;
const focusCSS=`
#title{font-family:'DM Sans',Arial,sans-serif!important;font-size:57px;font-weight:700}
#jev-panel{position:absolute;left:525px;top:316px;width:515px;height:437px;background:#e9dff5;border:1px solid #bea8d5;border-radius:5px;padding:28px}
#jev-label{font:13px 'DM Mono',monospace;letter-spacing:1px;color:#7254b6}
#jev-name{font:500 51px 'DM Mono',monospace;color:#604393;margin:19px 0 12px}
#jev-description{font-size:22px;line-height:1.3;margin-bottom:23px}
#jev-values{font:17px 'DM Mono',monospace;color:#756481}
.typed-row{display:flex;justify-content:space-between;align-items:center;border-top:1px solid #cdbce0;padding:11px 0;gap:14px}
.typed-row small{font:15px 'DM Mono',monospace;color:#78658b}
.typed-row b{font:500 24px 'DM Mono',monospace;color:#513579}
.line .op{color:#655a6b}.line.jev-op .op{color:#7254b6}.line.jev-op{background:#f3ecfa}.line.active{background:#ddc9f1}
`;
const manifest=[];
for(const [index,s] of shots.entries()){
 ...
 await p.addStyleTag({content:css+focusCSS});
 await p.evaluate(({s,index})=>{
  // Preserve the live VM and canvas; reframe them only for the recording.
  const root=document.createElement('div');root.id='film';root.innerHTML='<div class="brand"><b>JRISC / JEV OPCODES</b><span>8 REGISTERS · 16-BIT STATE</span></div><h1 id="title"></h1><p id="message"></p><div id="listing"><div class="listing-label">EXECUTING THE REAL PROGRAM</div></div><div id="jev-panel"><div id="jev-label">JEV INSTRUCTION</div><div id="jev-name"></div><div id="jev-description"></div><div id="jev-values"></div></div><div id="states"></div><div id="explain"></div><div class="note">REAL VM · FIXED DEMO RESPONSES · SLOW STEP-THROUGH</div><div class="progress"></div>';
  ...
  root.querySelector('.progress').style.width=((index+1)/4*100)+'%';
  for(let i=5;i<window.filmSource.length;i++) {const line=document.createElement('div');line.className='line';line.dataset.pc=i;const tokens=window.filmSource[i].split(' ');line.innerHTML='<span class="addr"></span><span class="op"></span><span class="args"></span>';line.children[0].textContent=String(i).padStart(2,'0');line.children[1].textContent=tokens.shift();if(['CHOOSE','SCORE','TEST','JUDGE','CONF'].includes(line.children[1].textContent))line.classList.add('jev-op');line.children[2].textContent=tokens.join(' ');document.getElementById('listing').append(line);}
  window.filmState=()=>{const v=window.jrisc.vm;return [v.pc,v.r[0],v.r[2],v.mem[0xe001],v.mem[0xe021]]};
  window.filmPaint=(before,after)=>{const names=['PC','R0','R2','E001 · ANSWER','E021 · ANSWER'];document.getElementById('states').replaceChildren(...names.map((name,i)=>{const el=document.createElement('div');el.className='state'+(before[i]!==after[i]?' changed':'');const small=document.createElement('small');small.textContent=name;const b=document.createElement('b');b.textContent=before[i]===after[i]?String(after[i]):`${before[i]} → ${after[i]}`;el.append(small,b);return el;}));};
  window.filmSemantic=(op, ready=false)=>{
    const bank=window.jrisc.vm.bank;
    document.getElementById('jev-label').textContent=op==='CONF'?'LOCAL CONFIDENCE READ':'JEV INSTRUCTION';
    document.getElementById('jev-name').textContent=op;
    document.getElementById('jev-description').textContent={CHOOSE:'Choose one option.',SCORE:'Measure a degree.',TEST:'Estimate a yes probability.',JUDGE:'Ask independent questions together.',CONF:'Read the last answer’s confidence.'}[op];
    const el=document.getElementById('jev-values');el.replaceChildren();
    if(!ready){el.textContent='Ready to execute';return;}
    const vals=op==='CONF'?[['confidence',window.jrisc.vm.r[2]+' / 10000']]:Object.entries(bank.answers).map(([key,a])=>[key,a.type==='choice'?a.choice:a.type==='score'?a.score.toFixed(2)+' / 2':Math.round(a.noul*100)+'% yes']);
    for(const [label,value] of vals){const row=document.createElement('div');row.className='typed-row';const small=document.createElement('small');small.textContent=label;const b=document.createElement('b');b.textContent=value;row.append(small,b);el.append(row);}
  };
  window.filmSemantic(window.filmDecode(window.jrisc.vm.words[5]).op);
 ...
 for(let n=0;n<20;n++){
  const step=await p.evaluate(async()=>{
   ...
   await vm.step();if(vm.error)throw Error(vm.error);const after=window.filmState();window.filmPaint(before,after);if(['CHOOSE','SCORE','TEST','JUDGE','CONF'].includes(d.op))window.filmSemantic(d.op,true);
   ...
   else if(d.op==='SCORE')note=`SCORE puts ${vm.r[d.a]} in R${d.a}. A typed value, ready for ordinary code.`;
   ...
   else if(d.op==='ST')note=(regs[d.b]+d.i===0xf001)?`ST writes palette ${vm.mem[0xf001]}. A normal memory write.`:`ST writes energy ${vm.mem[0xf004]}. A normal memory write.`;
  ...
  });steps.push(step);await p.waitForTimeout(({CHOOSE:1800,CONF:1800,SCORE:2300,TEST:2000,JUDGE:2700,BLT:1400,BEQ:750,LD:450,ST:400,JMP:350,LI:250})[step.op] || 400);await p.screenshot({path:`${dir}/${s.id}-${n}.png`});if(step.next===5)break;
 }
...
execFileSync(ff,['-y','-f','concat','-safe','0','-i',`${dir}/concat.txt`,'-c','copy','-movflags','+faststart','artifacts/jrisc-instruction-set.mp4'],{stdio:'pipe'});
execFileSync(ff,['-y','-ss','3','-i','artifacts/jrisc-instruction-set.mp4','-frames:v','1','artifacts/jrisc-instruction-set-cover.png'],{stdio:'pipe'});
console.log('Created artifacts/jrisc-instruction-set.mp4');
