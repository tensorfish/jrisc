...
// Illustrative typed fixtures, not live inference or keyword classification.
const demoValues = {
  choose: [
    ["warm", 1, 0.98],
    ["storm", 2, 0.02],
...
export function demoResponse(descriptor) {
  let values;
  for (const [kind, examples] of Object.entries(DEMO_EXAMPLES)) {
    const index = examples.findIndex(
      ([, message]) => message === descriptor.state.message,
    );
    if (index !== -1) {
      values = demoValues[kind][index];
      break;
    }
  }
  if (!values) throw Error("Choose one of the demo messages.");
  const [mood, energy, kindness] = values;
  const answers = Object.fromEntries(
    Object.entries(descriptor.questions).map(([id, q]) => {
      if (q.type === "choice" && ["mood", "intent"].includes(id)) {
      ...
      if (q.type === "score" && id === "energy")
        return [
          id,
          {
            ...
            probabilities: Object.fromEntries(
              q.criteria.map((_, i) => [i, i === energy ? 1 : 0]),
            ),
          },
        ];
      if (q.type === "noul" && id === "kindness")
        return [id, { type: "noul", noul: kindness }];
      throw Error("This question has no demo fixture.");
    }),
  );
  const result = {
    model: "local-demo",
