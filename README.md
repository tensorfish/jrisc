...
Explore the 20 opcode keys, then run CHOOSE (mood palette), SCORE (motion energy), TEST (kindness gate), or JUDGE (parallel judgments). Each example has contrasting inputs and shows its complete program. Open the machine to step, edit assembly, inspect registers or replay without another inference request. Browser requests go to `/api/proxy`; the local server supplies the TypeSafe credentials from `.env` and owns the question bank. Keys never enter the browser.
